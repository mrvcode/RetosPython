...
CLIENT_ID = "9fcdb0c499c44e98968ede0c5ff5c7a8"
...
def search_artist(token: str, name: str):
    ...
    response = requests.get(url, headers=headers)  # Changed from POST to GET
    if response.status_code != 200:
        raise Exception(f"Error obteniendo el artista: {response.json()}")
    results = response.json()
    if results["artists"]["items"]:
        return results["artists"]["items"][0]["id"]  # Changed from requests to results
    else:
        raise Exception(f"El artista {name} no se encuentra en Spotify.")
...
def get_artist_top_track(token: str, id: str):
    ...
    return {"name": top_track["name"], "popularity": top_track["popularity"]}
...
# 2. IDs
artist_1_id = search_artist(token, "Imagine Dragons")
...
if artist_1["followers"] > artist_2["followers"]:
    print(f"{artist_1["name"]} es mas popular en numero de seguidores.")
    artist_1_counter += 1
else:
    print(f"{artist_2["name"]} es mas popular en numero de seguidores.")
    artist_2_counter += 1
# 4.2 Popularidad
...
if artist_1["popularity"] > artist_2["popularity"]:
    print(f"{artist_1["name"]} es mas popular a nivel general.")
    artist_1_counter += 1
else:
    print(f"{artist_2["name"]} es mas popular a nivel general.")
    artist_2_counter += 1
...
if top_track_artist_1["popularity"] > top_track_artist_2["popularity"]:
    print(
        f"La canción {top_track_artist_1["name"]} de {artist_1["name"]} es mas popular."
    )
    artist_1_counter += 1
else:
    print(
        f"La canción {top_track_artist_2["name"]} de {artist_2["name"]} es mas popular."
    )
    artist_2_counter += 1
...
print(f"\nResultado final:\n")
print(
    f"{artist_1["name"] if artist_1_counter > artist_2_counter else artist_2["name"]} es mas popular"
)
